...
def diagonalround(y: list[Word], viz_round: bool) -> list[Word]:
    ...
    if viz_round:
        visualizer.qround(y, z, [0, 5, 10, 15])
...
def columnround(x: list[Word], viz_round: bool) -> list[Word]:
...
def doubleround(x: list[Word], viz_round: bool) -> list[Word]:
    ...
    if len(x) != 16:
        raise ValueError('x must be a 16-word list')
    visualizer.block(x)
    y = diagonalround(columnround(x, viz_round), viz_round)
    visualizer.block(y)
    return y
